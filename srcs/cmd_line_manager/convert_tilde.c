...
/*   Updated: 2022/04/29 18:06:12 by llethuil         ###   ########lyon.fr   */
/*                                                                            */
/* ************************************************************************** */
...
char	*convert_tilde(t_input *input, char *str)
{
	...
	while (str[i])
	{
		...
		{
			line = ft_strjoin(input, line, copy_in_quote(input, str, &i, &red));
			input->gb->type = GARBAGE;
		}
		else if (str[i] && str[i] != '~')
		{
...
static char *convert_actual_tilde()
{
	if (red == NO || red == YES)
			{
				if ((i == 0 && is_space(str[i + 1]) == YES)
					|| (is_space(str[i - 1]) == YES && is_space(str[i - 1]) == YES))
				{
