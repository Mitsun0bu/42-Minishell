...
/*   Updated: 2022/02/18 11:42:13 by llethuil         ###   ########lyon.fr   */
/*                                                                            */
/* ************************************************************************** */
...
int	malloc_output_redir_tab(t_input *input)
{
	...
	while (input->cmd_tab[++i_cmd])
	{
		...
		if (!n_red)
		{
			...
			// 	return (1);
			input->redir_tab[i_cmd][1] = safe_malloc(sizeof(char *), 2);
		...
		else
		{
			...
			// if (!input->redir_tab[i_cmd][1])
			// 	return (1);
			input->redir_tab[i_cmd][1] = safe_malloc(sizeof(char *), (n_red + 1));
